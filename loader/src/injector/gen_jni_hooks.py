...
fas_b = ForkAndSpec('b', [uid, gid, gids, runtime_flags, rlimits, mount_external, se_info,
    nice_name, fds_to_close, fds_to_ignore, is_child_zygote, instruction_set, app_data_dir, is_top_app,
    use_fifo_ui, pkg_data_info_list, whitelisted_data_info_list, mount_data_dirs, mount_storage_dirs,
...
fas_nubia_u = ForkAndSpec('nubia_u', [uid, gid, gids, runtime_flags, rlimits, mount_external,
    ...
    is_top_app, use_fifo_ui, pkg_data_info_list, whitelisted_data_info_list, mount_data_dirs,
    mount_storage_dirs, mount_sysprop_overrides, Anon(jstring)])
...
spec_xr_u = SpecApp('xr_u', [uid, gid, gids, runtime_flags, rlimits, mount_external, se_info, nice_name,
    is_child_zygote, instruction_set, app_data_dir, is_top_app, is_perception_app, pkg_data_info_list,
    whitelisted_data_info_list, mount_data_dirs, mount_storage_dirs])
...
spec_nubia_u = SpecApp('nubia_u', [uid, gid, gids, runtime_flags, rlimits, mount_external, se_info,
...
with open('jni_hooks.hpp', 'w') as f:
    ...
    methods = [fas_l, fas_o, fas_p, fas_q_alt, fas_r, fas_u, fas_b, fas_samsung_m, fas_samsung_n, fas_samsung_o, fas_samsung_p, fas_nubia_u]
    f.write(gen_jni_def(zygote, methods))

    methods = [spec_q, spec_q_alt, spec_r, spec_u, spec_xr_u, spec_samsung_q, spec_nubia_u]
    f.write(gen_jni_def(zygote, methods))
