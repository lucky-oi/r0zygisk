        ...
        for (;;) {
            ...
            switch (msg.cmd) {
                ...
                case SYSTEM_SERVER_STARTED:
                    ...
                    umount2("./module.prop", MNT_DETACH);
                    break;
            }
        }
    }
...
static void updateStatus() {
    auto zygote_state = [](const Status &status) -> const char * {
        if (!status.supported) {
            return "unsupported";
        }
        // Preserve the last confirmed injection result while tracing is stopped.
        // Re-starting the monitor does not immediately re-inject the current zygote;
        // a fresh zygote start is required for a new injection attempt.
        if (status.zygote_injected) {
            return "injected";
        }
        return tracing_state == TRACING ? "not_injected" : "unknown";
    };
...
#define WRITE_STATUS_ABI(suffix) \
    if (status##suffix.supported) { \
        ...
        else if (tracing_state == TRACING) status_text += "not injected,"; \
    ...
    WRITE_STATUS_ABI(64)
    WRITE_STATUS_ABI(32)
    auto status = xopen_file(status_path.c_str(), "w");
    fprintf(status.get(),
            "{\n"
            "  \"monitor\": \"%s\",\n"
            "  \"mode\": \"%s\",\n"
            ...
            json_escape(status64.daemon_running ? status64.daemon_info : status64.daemon_error_info).c_str(),
            zygote_state(status32),
            status32.supported ? (status32.daemon_running ? "running" : "crashed") : "unsupported",
            json_escape(status32.daemon_running ? status32.daemon_info : status32.daemon_error_info).c_str(),
            json_escape(status_text).c_str());
}
